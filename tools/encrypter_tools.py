...
async def encrypt(data:dict)->str:
    if "iat" in data:
        ...
        iat = (datetime.datetime.utcnow() + datetime.timedelta(hours=5, minutes=30)).isoformat().replace('-','').replace('T','')
    key = ""
    x = await generate_random_number()
    key += str(x) + (await generate_random_string(x)) + str(data['id']) + (await generate_splc_number())
    for y in iat:
        # if y == 'T':
        #     break
        x = await generate_random_number()
        key += str(x) + (await generate_random_string(x)) + y + (await generate_splc_number())
